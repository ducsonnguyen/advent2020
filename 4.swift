...
typealias FieldRequirement = (pattern: String, min: Int, max: Int)
let requirements: [String : [FieldRequirement]] = [
    "byr" : [("^[0-9]{4}$", 1920, 2002)],
    "iyr" : [("^[0-9]{4}$", 2010, 2020)],
    "eyr" : [("^[0-9]{4}$", 2020, 2030)],
    "hgt" : [("^[0-9]*cm$", 150, 193), ("^[0-9]*in$", 59, 76)],
    "hcl" : [("^#[0-9a-f]{6}$", 0, 0)],
    "ecl" : [("amb|blu|brn|gry|grn|hzl|oth", 0, 0)],
    "pid" : [("^[0-9]{9}$", 0, 0)],
]

func checkPassport1(fields: [String]) -> Bool {
    // check for presence of every required field
    let keys = fields.map() { $0.components(separatedBy: ":")[0] }
    return requirements.keys.reduce(true) { $0 && keys.contains($1) }
}

func checkField(_ field: (key: String, value: String)) -> Bool {
    guard let reqs = requirements[field.key] else {
        // print("Skipping key \(field.key)")
        return true
    }

    // just one requirement needs to pass
    for req in reqs {
        // check regex
        let range = NSRange(location: 0, length: field.value.utf16.count)
        let regex = try! NSRegularExpression(pattern: req.pattern)
        if regex.firstMatch(in: field.value, options: [], range: range) == nil {
            continue
        }

        // check minimum
        if req.min != 0 {
            if Int(field.value.filter("0123456789".contains))! < req.min {
                continue
            }
        }
        ...
        // check maximum
        if req.max != 0 {
            if Int(field.value.filter("0123456789".contains))! > req.max {
                continue
            }
        }
...
func checkPassport2(fields: [String]) -> Bool {
    if !checkPassport1(fields: fields) {
        return false
    }

    // check fields
    let parsedFields = fields.map() { $0.components(separatedBy: ":") }
    return parsedFields.reduce(true) { $0 && checkField(($1[0], $1[1])) }
}

func checkPassports(data: [String], checker: ([String]) -> Bool) -> Int {
    var validCount = 0
    var currentFields: [String] = []
    for line in data {
        if line.isEmpty {
            if checker(currentFields) {
                validCount += 1
            }
            currentFields = []
            continue
...
print("Part 1: \(checkPassports(data: data, checker: checkPassport1))")
print("Part 2: \(checkPassports(data: data, checker: checkPassport2))")
