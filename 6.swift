...
// c.  The second group contains three people; combined, they answered "yes" to 3
// questions: a, b, and c.  The third group contains two people; combined, they
// answered "yes" to 3 questions: a, b, and c.  The fourth group contains four
// people; combined, they answered "yes" to only 1 question, a.  The last group
// contains one person who answered "yes" to only 1 question, b.  In this example,
...
// and c.  In the second group, there is no question to which everyone answered
...
while let line = readLine() {
    if line.isEmpty {
        var all = questionsAnswered[0]
        var any = questionsAnswered[0]
        for q in questionsAnswered {
            all = all.intersection(q)
            any = any.union(q)
        }
        allAnswered += all.count
        anyAnswered += any.count
    ...
    questionsAnswered.append(person)
}

print("Any answered count: \(anyAnswered)")
print("All answered count: \(allAnswered)")
